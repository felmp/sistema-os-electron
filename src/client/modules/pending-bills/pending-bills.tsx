...
import { StatusPendingBill } from '../../components/status-os'
...
export default function PendingBills() {
  ...
  const filteredPendingBills = (titleFilter !== '' || dueDateFilter !== '__/__/____' || statusFilter !== '' || priceFilter !== '')
    ? pending_bills.data?.filter(f => {
      const titleCondition = titleFilter === '' || f.title.toLocaleLowerCase().includes(titleFilter.toLocaleLowerCase()) || f.title.toLocaleUpperCase().includes(titleFilter.toLocaleLowerCase())
      const dueDateCondition = dueDateFilter === '' || f.due_date.toLocaleLowerCase().includes(dueDateFilter.toLocaleLowerCase()) || f.due_date.toLocaleUpperCase().includes(dueDateFilter.toLocaleLowerCase());
      const statusCondition = statusFilter === '' || f.status.toLocaleLowerCase().includes(statusFilter.toLocaleLowerCase()) || f.status.toLocaleUpperCase().includes(statusFilter.toLocaleLowerCase());
      const priceCondition = priceFilter === '' || f.price.toString().includes(priceFilter);

      return titleCondition && dueDateCondition && statusCondition && priceCondition;
    })
    : pending_bills.data;
      ...
      <div className='flex w-full mt-4'>
        <div className="w-full h-auto bg-white rounded px-4 py-4 flex items-center font-normal text-xs gap-4">
          <div className='w-full flex flex-col'>
            ...
            <input className='w-full h-[32px] rounded pl-2 border border-slate-200 focus:outline-slate-300' type="text" name='plate' onChange={(e) => setTitleFilter(e.target.value)} />
          </div>
          <div className='w-full flex flex-col'>
            <label className='mb-3'>Vencimento</label>
            <InputMask
              mask="99/99/9999"
              className='w-full h-[32px] rounded pl-2 border border-slate-200 focus:outline-slate-300' type="text" name='model' onChange={(e) => setDueDateFilter(e.target.value)} />
          </div>
          <div className='w-full flex flex-col'>
            <label className='mb-3'>Status</label>
            <select className='w-full h-[32px] rounded pl-2 border border-slate-200 focus:outline-slate-300' name="status" onChange={(e) => setStatusFilter(e.target.value)}>
              ...
              currentItems?.map(s => {
                return (
                  <div onClick={() => handlePendingBillsDetails(s.id)} key={s.id} className='hover:bg-slate-200 cursor-pointer table-row'>
                    ...
                    <div className='text-left p-[16px] table-cell'><StatusPendingBill status={s.status} /></div>
                  </div>
                )
              })
            }
