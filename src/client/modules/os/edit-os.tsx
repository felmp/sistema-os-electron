...
export default function EditOs() {
  ...
  function removeService(index: number) {
    setDeletedServices((prevDeletedServices) => [
      ...prevDeletedServices,
      fields[index].cod ?? "",
    ]);
  ...
  useEffect(() => {
    if (os.data && services.data) {
      const formattedData = {
        ...
        observation: os.data.observation || "",
      ...
      setTotalPrice(
        services.data.map((i) => i.price).reduce((a, t) => a + t, 0)
      );
    }
    ...
    <div className="px-6 pt-8 w-full h-full">
      ...
      <form onSubmit={handleSubmit(handlerUpdateOs)}>
        <div className="w-full h-auto mb-3 mt-4 bg-white rounded p-4">
          ...
          <div className="flex flex-row font-normal gap-4 mt-4">
            <div className="w-full flex flex-col">
              <label className="mb-3">Observação</label>
              <textarea
                rows={4}
        ...
        <div className="h-10 w-full flex-row flex justify-between">
          ...
          <div className="flex flex-row">
            <button
              ...
              className="flex items-center justify-center font-normal  text-white w-[69px] h-10 p-4 bg-red-500 rounded"
            >
              Deletar
            </button>
          </div>
